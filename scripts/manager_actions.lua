...
function performAction(draginfo, rActor, rRoll, bOverride)
	if not rRoll then
		return;
	end
	
	performMultiAction(draginfo, rActor, rRoll.sType, { rRoll }, bOverride);
end

function performMultiAction(draginfo, rActor, sType, rRolls, bOverride)
...
function actionDirect(rActor, sDragType, rRolls, aTargeting)
	if GameSystem.actions[sDragType].sTargeting then
...
function encodeActors(draginfo, rSource, aTargets)
	if rSource then
		draginfo.addShortcut(rSource.sType, rSource.sCreatureNode);
	else
	...
	if aTargets then
		for _,v in ipairs(aTargets) do
			draginfo.addShortcut(v.sType, v.sCreatureNode);
		end
	end
end
...
function encodeActionForDrag(draginfo, rSource, sType, rRolls)
	...
	if GameSystem.actions[sType].sIcon then
...
function buildThrow(rSource, vTargets, rRoll, bMultiTarget)
	...
	rThrow.shortcuts = {};
	if rSource then
		table.insert(rThrow.shortcuts, { class = rSource.sType, recordname = rSource.sCreatureNode });
	else
	...
	if vTargets then
		if bMultiTarget then
			for _,v in ipairs(vTargets) do
				table.insert(rThrow.shortcuts, { class = v.sType, recordname = v.sCreatureNode });
			end
		else
			table.insert(rThrow.shortcuts, { class = vTargets.sType, recordname = vTargets.sCreatureNode });
		end
	end
...
function createActionMessage(rSource, rRoll)
	...
	if rRoll.bSecret then
		...
		if string.match(sDesc, "^%[TOWER%] ") then
...
function copyRoll(rOldRoll)
	local rNewRoll = {};
	
	for k,v in pairs(rOldRoll) do
		if k == "aDice" then
			rNewRoll.aDice = {};
			for _,vDie in ipairs(v) do
...
function copyRolls(rRolls)
	local rNewRolls = {};
	for _,vRoll in ipairs(rRolls) do
		table.insert(rNewRolls, copyRoll(vRoll));
	end
